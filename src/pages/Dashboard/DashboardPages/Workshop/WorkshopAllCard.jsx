    ...
    return (
        <div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-6">
            {
                ...
                <h1 className="text-lg text-[#F9F5FF] font-bold font-roboto mb-4">{details?.courseName}</h1>


                <div className="flex justify-between items-center mb-5">
                <div className="flex items-center gap-2">
                    <img className="w-6" src={calender} alt="" />
                    <p className="text-base text-[#C7C4CC] font-light font-roboto">{details?.enrollDate}</p>
                </div>

                <div className="flex items-center gap-2">
                    <img src={clock} alt="" />
                    <p className="text-base text-[#C7C4CC] font-light font-roboto">{details.time}</p>
                </div>
                </div>

                <div className="flex justify-between items-center">
                <div>
                <h1 className="text-base text-[#AEABB2] font-normal font-roboto mb-[3px]">It will start in</h1>
                <p className="text-[18px] text-[#F9F5FF] font-bold font-roboto">{details?.startIn}</p>
                </div>
