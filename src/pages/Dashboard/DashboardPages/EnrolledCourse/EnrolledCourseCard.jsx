...
const EnrolledCourseCard = () => {
    ...
    // grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4
    return (
        <div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
            {
                courseDetails.map((details, index) => 
                <div key={index} className="w-[318px] border border-[#4B494D] rounded-2xl">
                <img className="rounded-t-2xl mb-2 w-full" src={details?.img} alt="" />

                <div className="px-4 pb-2">
                <h1 className="text-lg text-[#F9F5FF] font-bold font-roboto mb-[3px]">{details?.courseName}</h1>

                <div className="flex items-center gap-2 mb-4">
                    <img src={calender} alt="" />
                    <p className="text-[12px] text-[#F9F5FF] font-normal font-roboto">{details?.enrollDate}</p>
                </div>

                <div className="flex items-center gap-2 mb-2">
                    <img src={timer} alt="" />
                    <p className="text-base text-[#F9F5FF] font-normal font-roboto">{details.time}</p>
                </div>

                <div className="flex items-center justify-between gap-2 mb-4">
                <progress className="progress progress-primary w-56" value={details.completedModule} max="100"></progress>
                    <p className="text-base text-[#F9F5FF] font-normal font-roboto">{details.completedModule}%</p>
                </div>
